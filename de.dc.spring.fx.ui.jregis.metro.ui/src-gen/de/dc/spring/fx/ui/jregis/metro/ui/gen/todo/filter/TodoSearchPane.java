...
public class TodoSearchPane extends VBox{
    ...
    @FXML
    protected ComboBox<TodoSearchType> comboboxName;
    ...
    @FXML
    protected ComboBox<TodoSearchType> comboboxStatus;
    ...
    @FXML
    protected ComboBox<TodoSearchType> comboboxCreatedOn;
    ...
    @FXML
    protected ComboBox<TodoSearchType> comboboxUpdatedOn;

    @FXML
    protected TextField textUpdatedOn;
    
    private ObservableList<TodoSearchContent> content = FXCollections.observableArrayList();
     
	public static final String FXML = "/de/dc/spring/fx/ui/jregis/metro/ui/gen/todo/filter/TodoSearchPane.fxml";

	public TodoSearchPane() {
    ...
    protected void onActionEvent(ActionEvent event) {
    	if (event.getSource()==buttonAccept) {
			...
			if(checkboxName.isSelected()){
				...
				TodoSearchType type = comboboxName.getSelectionModel().getSelectedItem();
				content.add(new TodoSearchContent(name, value, type));
			}
			if(checkboxStatus.isSelected()){
				String value = textStatus.getText();
				String name = checkboxStatus.getText();
				TodoSearchType type = comboboxStatus.getSelectionModel().getSelectedItem();
				content.add(new TodoSearchContent(name, value, type));
			}
			if(checkboxCreatedOn.isSelected()){
				String value = textCreatedOn.getText();
				String name = checkboxCreatedOn.getText();
				TodoSearchType type = comboboxCreatedOn.getSelectionModel().getSelectedItem();
				content.add(new TodoSearchContent(name, value, type));
			}
			if(checkboxUpdatedOn.isSelected()){
				String value = textUpdatedOn.getText();
				String name = checkboxUpdatedOn.getText();
				TodoSearchType type = comboboxUpdatedOn.getSelectionModel().getSelectedItem();
				content.add(new TodoSearchContent(name, value, type));
			}
