package de.dc.fx.ui.jregis.metro.ui.gen.contacts.contact.init;

import java.io.*;
import java.net.URL;
import java.sql.*;

import org.h2.tools.RunScript;

import de.dc.fx.ui.jregis.metro.ui.gen.contacts.contact.repository.*;
public class ContactTableInitializer{
	public static void main(String[] args) throws SQLException, FileNotFoundException {
		Connection connection = DriverManager.getConnection(ContactRepository.JDBC_URL,"SA", "SA");
		URL createUrl = ContactTableInitializer.class.getResource("/de/dc/fx/ui/jregis/metro/ui/gen/contacts/contact/init/create.sql");
		RunScript.execute(connection, new FileReader(new File(createUrl.getFile())));
		
		System.out.println("Tables initialized!");
	}
}
